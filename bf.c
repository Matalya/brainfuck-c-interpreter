...
int main(){
    ...
    while (rdr < CODE_LEN) {
        ...
        if (current == '[') {
            ...
            if (tape[ptr] != 0) {
                rdr = getStackTop(activeBrackets) - 1;
            }
        ...
        //printf("stack len = %ld; stack top = %d; rdr=%d on %c; ptr: %d; wait: %d\n", activeBrackets->size, getStackTop(activeBrackets), rdr, CODE[rdr], ptr, wait);
        rdr++;
        usleep(wait);
        wait = 2500;
    }
